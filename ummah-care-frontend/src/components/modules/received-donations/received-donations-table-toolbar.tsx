...
import { statuses } from "@/components/modules/received-donations/received-donations-table-data";
...
interface ReceivedDonationsTableToolbarProps<TData> {
  table: Table<TData>;
}

export function ReceivedDonationsTableToolbar<TData>({
  table,
}: ReceivedDonationsTableToolbarProps<TData>) {
  const {
    search,
    setSearch,
    filterValues,
